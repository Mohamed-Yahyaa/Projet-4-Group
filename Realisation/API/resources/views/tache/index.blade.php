            ...
            <div class="table-title">
                ...
                <div class="col-sm-12 d-flex justify-content-between p-3">
                    <div class="d-flex justify-content-between">
                        ...
                        <select class="btn btn-secondary dropdown-toggle ml-2" name="filterTache" id="filterTache">
                            <option value="">{{__('message.all_briefs')}}</option>
                            @foreach ($tasks as $value)
                            <option value="{{$value->id}}">{{$value->Etat}}</option>
                            @endforeach
                        </select>
    ...
    <table class="table table-striped table-hover table-bordered">
      ...
      <tbody  class="table1" id="table1">
          ...
          <tr>
              ...
              <td>{{ $task->Etat }}</td>
              <td >
                  <a  href="{{ route('tache.edit', $task->id)}}" class="edit" title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a>

                    @can("isFormateur")
                 <form action="{{ route('tache.destroy', $task->id)}}" method="post">
                    @csrf
                    @method('DELETE')
